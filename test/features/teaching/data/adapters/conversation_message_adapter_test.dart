...
void main() {
  group('ConversationMessageAdapter', () {
    ...
    test('write/read round-trip with all fields', () {
      ...
      expect(restored.toolCallId, 'call_test');
      expect(restored.toolName, 'test_tool');
      expect(restored.toolArguments, '{"arg": 1}');
      expect(restored.toolResult, '{"result": "ok"}');
    });
  ...
  group('ConversationMessageAdapter edge cases', () {
    ...
    test('write/read round-trip with tool call fields', () {
      final registry = TypeRegistryImpl()
        ..registerAdapter(ConversationMessageAdapter());
      final adapter = ConversationMessageAdapter();
      final now = DateTime.now();
      ...
      expect(restored.toolCallId, 'call_abc123');
      expect(restored.toolName, 'get_weather');
      expect(restored.toolArguments, '{"location": "Tokyo"}');
      expect(restored.toolResult, isNull);
    });

    test('write/read round-trip with tool result', () {
      final registry = TypeRegistryImpl()
        ..registerAdapter(ConversationMessageAdapter());
      final adapter = ConversationMessageAdapter();
      final now = DateTime.now();
      ...
      expect(restored.toolCallId, 'call_xyz');
      expect(restored.toolName, 'get_weather');
      expect(restored.toolArguments, '{"location": "Paris"}');
      expect(restored.toolResult, '{"temp": 22, "unit": "C"}');
    });

    test('write/read round-trip with all tool fields null', () {
      final registry = TypeRegistryImpl()
        ..registerAdapter(ConversationMessageAdapter());
      final adapter = ConversationMessageAdapter();
      final now = DateTime.now();
  ...
  group('TutorSessionAdapter edge cases', () {
    ...
    test('write/read round-trip with lessonId', () {
      final registry = TypeRegistryImpl()
        ..registerAdapter(TutorSessionAdapter());
      final adapter = TutorSessionAdapter();
      final now = DateTime.now();
    ...
    test('write/read round-trip with lessonId null', () {
      final registry = TypeRegistryImpl()
        ..registerAdapter(TutorSessionAdapter());
      final adapter = TutorSessionAdapter();
      final now = DateTime.now();
