  ...
  group('QuestionRepository', () {
    ...
    group('get', () {
      test('returns question by id', () async {
        ...
        final result = await repository.get('q-1');
        expect(result.isSuccess, isTrue);
        expect(result.data?.text, 'Test question?');
      });

      test('returns success with null data for missing question', () async {
        final result = await repository.get('not-found');
        expect(result.isSuccess, isTrue);
    ...
    group('delete', () {
      test('removes question', () async {
        await repository.create(createTestQuestion(id: 'q1'));
        final result = await repository.delete('q1');
        expect(result.isSuccess, isTrue);
