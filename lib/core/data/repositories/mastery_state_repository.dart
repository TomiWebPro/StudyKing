...
class MasteryStateRepository extends Repository<MasteryState> {
  MasteryStateRepository() : super(boxName: HiveBoxNames.masteryStates);
  static final Logger _logger = const Logger('MasteryStateRepository');

  Future<Result<void>> init() async {
    return Result.capture(() async {
      await openBox(HiveBoxNames.masteryStates);
    }, context: 'MasteryStateRepository.init');
  }
  ...
  Future<Result<MasteryState>> getMasteryState(
    ...
    try {
      ...
      _logger.w('Error getting mastery state', e);
  ...
  Future<Result<void>> updateMasteryState(MasteryState state) async {
    try {
      ...
      _logger.w('Error updating mastery state', e);
      return Result.failure(e.toString());
    }
  }
  ...
  Future<Result<List<MasteryState>>> getAllMasteryStates(
    ...
    try {
      ...
      _logger.w('Error getting all mastery states', e);
      return Result.failure(e.toString());
    }
  }
  ...
  Future<Result<List<MasteryState>>> getTopicsNeedingReview(
    ...
    try {
      ...
      _logger.w('Error getting topics needing review', e);
      return Result.failure(e.toString());
    }
  }
  ...
  Future<Result<List<MasteryState>>> getWeakTopics(String studentId) async {
    try {
      ...
      _logger.w('Error getting weak topics', e);
      return Result.failure(e.toString());
    }
  }
