...
class SpacedRepetitionQueries {
  ...
  static List<Question> getQuestionsDueForReview(Box<Question> box, {DateTime? asOf}) {
    final reviewDate = asOf ?? DateTime.now();
    final cutover = reviewDate.subtract(const Duration(hours: 1)); // 1 hour buffer
    final all = box.values.toList();
    all.sort((a, b) => (a.nextReview ?? DateTime.now()).compareTo(b.nextReview ?? DateTime.now()));
    return all.where((q) => (q.nextReview ?? DateTime.now()).isBefore(cutover)).toList();
  }

  /// Get questions due after specific threshold
  static List<Question> getQuestionsDueAfter(Box<Question> box, DateTime asOf) {
    final cutover = asOf.subtract(const Duration(minutes: 30));
    return box.values.where((q) => (q.nextReview ?? DateTime.now()).isBefore(cutover)).toList();
  }

  /// Check if a question is due for review (due within last 5 minutes)
  ...
  /// Convert list of questions to box-safe format for sorting
  static List<Question> questionsToBoxSafe(List<Question>? questions) {
    return questions ?? [];
  }

  /// Count total questions in box
  static int countAllQuestions(Box<Question> box) {
    return box.values.length;
  }

  /// Get questions with their due status
  static Map<String, String> mapQuestionsToStatus(Box<Question> box, {DateTime? asOf}) {
    final reviewDate = asOf ?? DateTime.now();
    final cutover = reviewDate.subtract(const Duration(hours: 1));
    return {
      for (final q in box.values)
        q.id: (q.nextReview ?? DateTime.now()).isBefore(cutover) ? 'due' : 'not-due',
    };
  }
}
...
class SpacedRepetitionRepository {
  ...
  Future<Result<List<DateTime>>> getQuestionDueTimes(String questionId) async {
    try {
      ...
      // Extract next_review timestamps from attempts due timestamps
  ...
  Future<Result<List<Question>>> getPracticeQuestions(String subjectId) async {
    try {
      ...
      final all = _questionBox.values.toList();
      final practiceQuestions = all.where((q) => (q.nextReview ?? DateTime.now()).isBefore(DateTime.now()) && q.subjectId == subjectId);
  ...
  Future<Result<int>> getSubjectDueCount(String subjectId) async {
    try {
      ...
      final all = _questionBox.values.toList();
      final dueCount = all.where((q) => q.subjectId == subjectId && (q.nextReview ?? DateTime.now()).isBefore(DateTime.now())).length;
