...
class MasteryGraphRepository {
  ...
  Future<Result<MasteryState>> getMasteryState(String studentId, String topicId) async {
    try {
      ...
      final state = _masteryBox.get(key);
      if (state != null) {
  ...
  Future<Result<List<MasteryState>>> getAllMasteryStates(String studentId) async {
    try {
      final states = _masteryBox.values
  ...
  Future<Result<List<QuestionMasteryState>>> getDueQuestions(
    ...
    try {
      ...
      final states = _questionMasteryBox.values
          .where((s) => s.studentId == studentId && s.nextReview != null && s.nextReview!.isBefore(now))
          .toList();
  ...
  Future<Result<List<QuestionMasteryState>>> getAtRiskQuestions(
    ...
    try {
      final states = _questionMasteryBox.values
          .where((s) => s.studentId == studentId && s.masteryLevel < threshold)
          .toList();
  ...
  Future<Result<Map<String, dynamic>>> getMasterySnapshot(String studentId) async {
    try {
      ...
      final avgAccuracy = topicStates.isEmpty
          ? 0.0
          : topicStates.map((s) => s.accuracy).reduce((a, b) => a + b) / topicStates.length;

      final masteredTopics = topicStates.where((s) => s.masteryLevel.index >= 3).length;
      final weakTopics = topicStates.where((s) => s.accuracy < 0.6).length;
      final totalAttempts = topicStates.fold<int>(0, (sum, s) => sum + s.totalAttempts);
      ...
      return Result.success({
        ...
        'avgReadiness': topicStates.isEmpty
            ? 0.0
            : topicStates.map((s) => s.readinessScore).reduce((a, b) => a + b) / topicStates.length,
