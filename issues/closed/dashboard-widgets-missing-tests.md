...
## Expected vs Actual
- Expected: `test/features/dashboard/presentation/widgets/syllabus_breakdown_card_test.dart` and `syllabus_switcher_test.dart` exist, using `ProviderScope` with overrides (fake repos / fixedStudentId) and asserting rendered state.
...
## Resolution
Added `syllabus_breakdown_card_test.dart` (renders empty state, one row per syllabus, completion %, accuracy/weak/study-time metrics, selected-syllabus filtering) and `syllabus_switcher_test.dart` (renders "All" + per-syllabus chips, default selection, tap-to-select behavior, selection persistence via `dashboardSelectedSyllabusProvider`). `flutter analyze` is clean and the two new test files pass.
